...
export default function App({ loaderData }: Route.ComponentProps) { const user = loaderData.user; return <><header className="site-head"><Link className="brand" to="/">Re:Taste<span>광주·전남 맛 지도</span></Link><nav><Link to="/places">장소 목록</Link>{user ? <><Link to="/me">내 취향</Link>{user.role === "REVIEWER" && <Link to="/reviewer/ratings">리뷰어 평가</Link>}{user.role === "ADMIN" && <Link to="/admin/candidates">어드민</Link>}<span className="text-sm font-medium text-emerald-800">{user.displayName}님</span><Form method="post" action="/logout"><button className="text-sm font-medium">로그아웃</button></Form></> : <Link to="/login">로그인</Link>}</nav></header><Outlet/><footer className="site-footer"><strong>Re:Taste</strong><span>추천할 만한 한 끼를 기록합니다.</span><nav aria-label="법적 고지"><Link to="/privacy">개인정보 처리방침</Link><Link to="/terms">이용약관</Link></nav></footer></>; }
