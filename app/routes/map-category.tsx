...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  return { places, state, categorySlug: params.categorySlug };
}
    ...
    <main id="main" className="explore-page">
      ...
      <div className="explore-split" data-view={loaderData.state.view}>
        <section className="map-panel" aria-label="지도 보기">
          <PlaceMap places={loaderData.places} selected={selected} onSelect={(id) => { setState("selected", id); document.getElementById(`place-${id}`)?.scrollIntoView({ behavior: "smooth" }); }} onBounds={(bbox) => setState("bbox", bbox.map((n) => n.toFixed(5)).join(","))} />
        </section>
