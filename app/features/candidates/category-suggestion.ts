...
const nameRules: Rule[] = [
  ...
  { pattern: /스시|초밥|횟집|회집|사시미/, slug: "sushi-sashimi", group: "japanese", label: "상호의 초밥·회 표현", kind: "FOOD" },
...
function signalScore(origin: SignalOrigin, kind: SignalKind) {
  if (origin === "NAME") return kind === "FOOD" ? 100 : 55;
  if (origin === "SUBTYPE") return kind === "FOOD" ? 70 : kind === "CUISINE" ? 30 : 20;
  return 5;
}
...
function collectSignals(input: { sourceType: PublicDataSource; businessSubtype?: string | null; businessName: string }) {
  ...
  const signals: Signal[] = [
    ...nameRules.filter((rule) => rule.pattern.test(normalizedName)).map((rule) => ({ ...rule, origin: "NAME" as const, score: signalScore("NAME", rule.kind) })),
    ...subtypeRules.filter((rule) => rule.pattern.test(subtype)).map((rule) => ({ ...rule, origin: "SUBTYPE" as const, score: signalScore("SUBTYPE", rule.kind) })),
  ];
...
export function classifyCandidate(input: {
  ...
  const hasCuisineConflict = primary.origin === "NAME" && primary.kind === "FOOD" && subtypeCuisine != null && subtypeCuisine.group !== primary.group;
